...
PhysTwin fits a small 3D rigid-body scene by running Newton on every objective
step. It does not infer a general physical scene. The input is a
`PhysicalScene` template. The output is a fitted `PhysicalScene` plus a
`SimulatedWorldState`.
...
- `tether_initial_tangent_velocity_fixed_length_v1` holds `rest_length_m`
  ...
  [real-video-reconstruction.md](real-video-reconstruction.md).
- `free_fall_gravity_v1` fits gravity magnitude and initial vertical velocity
  for one unconstrained sphere.

The tether length-fitting profile uses:

- `rest_length_m`: bounds `[1.6, 2.4]`, initial search value `1.78`
- `initial_tangent_velocity_u_m_s`: bounds `[-0.6, 0.6]`, initial `-0.12`
- `initial_tangent_velocity_v_m_s`: bounds `[-0.6, 0.6]`, initial `0.14`
...
Each tether candidate moves the initial body transform along `r` so the body
...
gravity-only ideal tether, mass does not change the trajectory. Damping is not
fitted because the validated runtime has no damping parameter.

`free_fall_gravity_v1` uses:

- `gravity_magnitude_m_s2`: bounds `[2, 20]`, initial `6.0`
- `initial_velocity_y_m_s`: bounds `[-6, 2]`

A kinematic parabola on observed Y is only the search start. Every evaluation
still runs Newton. IRIS gravity is not an initial value or a bound.
...
Synthetic evidence comes from a validated Newton rollout. The generator
decimates the saved rollout. It does not integrate a second model.

A `SceneObservation` is accepted only when all of these checks pass:
...
- the `humans.v1` pelvis track is valid, or an entity track is valid
...
- entity motion needs `0.02 m` AABB-diagonal travel. Planar paths are allowed

Scale is applied before `T_scene_observation_m`.
...
eligible for the human-root adapter. The CLI writes an `InversePhysicsFit` with
status `BLOCKED_INPUT` and the exact failed checks. It does not run an
optimizer or report fitted values.

## Objective and search

For observed positions `p_i`, Newton positions `q_i`, and weights `w_i`:
...
The report includes MSE, RMSE, observed 3D bounding-box diagonal, RMSE divided
by that diagonal, MSE at the declared initial search point, and the
improvement ratio from that point.

The optimizer is fixed-seed bounded differential evolution followed by
coordinate refinement. The default seed is `1347705141`. The default tether
budget is 8 population members, 4 generations, and 12 coordinate iterations.

The objective path records only body positions at requested times. The final
candidate still runs through the full rollout writer and validator.

## Synthetic tether recovery
...
The output directory contains `truth_physical_scene.json`,
`target_motion_observation.json`, `fitted_physical_scene.json`,
`simulated_world_state.json`, and `inverse_physics_fit.json`.
...
Same isolated environment as the Newton runtime:
...
In the diagnostics UI, blue points are `PhysicalMotionObservation`. The orange
path is the fitted `SimulatedWorldState`. Three.js does not integrate physics.

## Limits

The fitter recovers parameters for one passive rigid sphere, with either one
fixed-distance world tether or free fall. It does not fit anchor position,
mass, body orientation, damping, forces, contacts, articulated motion, or
topology.
